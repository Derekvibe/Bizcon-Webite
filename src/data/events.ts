export interface EventType {
  id: string;
  year: string;
  theme: string;
  date: string;
  location: string;
  description: string;
  images: string[];
  videos: string[];
  attendees?: number;
  speakers?: number;
  featured?: boolean;
}

export const events: EventType[] = [
  {
    id: "bizcon-2026",
    year: "2026",
    theme: "Empowering Business Growth",
    date: "2026-08-22",
    location: "Abakaliki, Nigeria",
    description:
      "BIZCON 2026 equips entrepreneurs and business managers with branding, marketing, and growth strategies to scale sustainably. Join 250+ business leaders/ Entrepreneurs 9 Speakers for Nigeria's most impactful one-day conference.",
    images: ["https://i.postimg.cc/mD8QVhRV/IMG-0772-JPG.jpg"],
    videos: ["https://www.youtube.com/embed/zAhFvMko4pE?si=adV2_Y9bwiayMKPd"],
    attendees: 300,
    speakers: 9,
    featured: true,
  },
  {
    id: "bizcon-2025",
    year: "2025",
    theme: "Small Business, Big Vision",
    date: "2025-09-20",
    location: "Abakaliki, Nigeria",
    description:
      "BIZCON 2025 equips entrepreneurs and business managers with branding, marketing, and growth strategies to scale sustainably. Join 500+ business leaders for Nigeria's most impactful one-day conference.",
    images: ["https://i.postimg.cc/mD8QVhRV/IMG-0772-JPG.jpg"],
    videos: ["https://www.youtube.com/embed/3l1YAF5q9z0?si=r6ONeVvQVntaJu2Y"],
    attendees: 500,
    speakers: 8,
    featured: true,
  },
  {
    id: "alumni_Hangout_2025",
    year: "2025",
    theme: "Alumni Hangout 2025",
    date: "2025-12-18",
    location: "Abakaliki, Nigeria",
    description:
      "BIZCON 2025 Alumni Hangout focused on strategic scaling models and operational excellence. Over 300 founders and managers attended sessions on lean operations, funding strategies, and sustainable growth.",
    images: ["/images/events/2025-1.jpg", "/images/events/2025-2.jpg"],
    videos: ["https://www.youtube.com/embed/3l1YAF5q9z0"],
    attendees: 320,
    speakers: 8,
  },
];
